...
public class Root_Search extends AppCompatActivity
{
    ...
    public void Select_Cursor()
    {
        ...
        switch (BUTTON_SELECT_FLG)
        {
            ...
            //未検針データ検索
            case SEARCH_FLG1:
                cursor = kenshin_db.db.rawQuery("SELECT G_code ,  K_jun , C_name1 , C_name2 , customer , P_name , ban FROM TOKUIF WHERE not P_flag = ? ", new String[]{"1"});
                Log.d("Cursor",String.valueOf(cursor.getCount()));
                break;
    
            //得意先コード検索
            case SEARCH_FLG2:
                //cursor = kenshin_db.db.rawQuery("SELECT G_code ,  K_jun , C_name1 , C_name2 , customer , P_name , ban FROM TOKUIF WHERE not P_flag = ? AND customer = ? ", new String[]{"1", customer_code.getText().toString()});
                cursor = kenshin_db.db.rawQuery("SELECT G_code ,  K_jun , C_name1 , C_name2 , customer , P_name , ban FROM TOKUIF WHERE customer = ? ", new String[]{customer_code.getText().toString()});
                Log.d("Cursor",String.valueOf(cursor.getCount()));
                break;
    
            //群コード検索
            case SEARCH_FLG3:
                //cursor = kenshin_db.db.rawQuery("SELECT G_code ,  K_jun , C_name1 , C_name2 , customer , P_name , ban FROM TOKUIF WHERE not P_flag = ? AND G_code = ? ", new String[]{"1", G_code.getText().toString()});
                cursor = kenshin_db.db.rawQuery("SELECT G_code ,  K_jun , C_name1 , C_name2 , customer , P_name , ban FROM TOKUIF WHERE G_code = ? ", new String[]{G_code.getText().toString()});
                Log.d("Cursor",String.valueOf(cursor.getCount()));
                break;
            ...
            case SEARCH_FLG4:
                ...
                //    "WHERE not P_flag = ? AND G_code = ? AND K_jun = ? ", new String[]{"1", Group_K_jun_C.getText().toString(), (Group_K_jun_G.getText().toString())});
                
                cursor = kenshin_db.db.rawQuery("SELECT G_code ,  K_jun , C_name1 , C_name2 , customer , P_name , ban FROM TOKUIF " +
                    "WHERE G_code = ? AND K_jun = ? ", new String[]{Group_K_jun_C.getText().toString(), (Group_K_jun_G.getText().toString())});
    ...
    public void Cursor_VIEW()
    {
        ...
        try
        {
            ...
            K_jun.setText(cursor_data_input[Array_line][1]);        //検針順
                                                                    //顧客名
            Name1.setText(String.format("%s%s",  cursor_data_input[Array_line][2],cursor_data_input[Array_line][3]));
